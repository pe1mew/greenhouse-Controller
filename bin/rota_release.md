...
Creating the release needs a token with **Contents: Read and write** on the repo
(the Issues-scoped `gh_issue.py` token is *not* sufficient). Keep it in the
operator's secret store and point `ROTA_TOKEN_FILE` (in git-ignored
`bin/.rota_release.env`) at it; resolution falls back to `$GITHUB_TOKEN`, then
`.github/token.local`. The VPS-side retriever (`ota-store-update.sh`) is step 2.
