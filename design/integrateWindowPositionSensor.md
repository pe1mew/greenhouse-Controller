...
> | clean aperture data over a sustained period | **PARTLY, and materially stronger as of 2026-09-15.** **Two clean traverses in both directions**: an OPEN ramp (45 samples, full 0 -> 15 000, 0 rejected) and a CLOSE ramp (98 accepted samples, 7 989 -> 122 monotonic, 0 rejected). **AT-WP09 passed both halves** — the divergence reported, and a healthy stroke not false-tripped, with rule 2 silent on the same CLOSE. What remains for *sustained* is elapsed time rather than any missing capability: **two traverses in one evening is not a period**, and **AT-WP07 is still unrun** (`bin/at_wp07.py`; needs the encoder disconnected at the bus, M3 open, and a wind trigger at the emulator). |
>
