...
| R-T07 | Deployment and publishing **shall** use SSH public-key authentication with host-key verification enabled; no passwords, private keys, or device secrets **shall** be stored in either repository. The server is deployed via a **git clone in the VPS user's home directory** (VPS → GitHub, read-only deploy key, pinned host key), updated by fast-forward `git pull` and **deployed from there** by a local copy of `public/` into the webroot; nginx serves only the deployed copy, so `.git/`, tooling, and config are never web-served. Release-artefact publishing (this repo → VPS `ota-store/`) uses SSH-key auth with `StrictHostKeyChecking` on. | Must | Server clone authenticates with a read-only key; publish succeeds only with the operator's key; credential-file scan clean in both repos; `.git/` not reachable under the OTA host. |
