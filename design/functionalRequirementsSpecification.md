...
| FR-LG06 | The system **shall** retain event log entries persistently on an SD card using a rotating-file strategy. Each file **shall** be capped at a maximum size (default 1 024 KB); when the file reaches the cap, the system **shall** close it and open a new file timestamped with the rotation moment. A bounded number of rotated files **shall** be retained (default 30), the oldest file being deleted automatically when the cap is exceeded. The bound is **per unit**: where one card collects the histories of more than one controller — the development rig, whose modules are swappable — each unit retains its own files and deletes only its own oldest *(clarified 2026-09-24, gh#82; on a card with a single unit this is the same rule as before)*. The rotation cap **shall** be sized such that worst-case activity at the minimum poll interval keeps the on-card retention measured in weeks-to-months even under load. | Must |
