...
Full task graph diagram in [`../design/rtosTaskDiagram.png`](../design/rtosTaskDiagram.png). Roles verified against `xTaskCreate*` sites in `firmware/src/main.cpp` (T1-T11, T14, T16, T17) and on-demand spawn points (T13, T15):
...
| T3 | `safety_monitor` | **Wind safety**: raises and clears `EG1_BIT_WIND_OVERRIDE` and posts CLOSE_ALL / RESUME to Q1. It safe-fails on a wind-sensor fault. **The motor alarm is T2's**, via the GPIO42 ISR — this row said "motor-alarm detection" until 2026-09-17, which is wrong and had been read as fact |
...
| T6 | `climate_control` | Mode + setpoint logic; consumes T4 notifications. **Its decision law moves behind `drivers/ventModel/` in 2.12.0** (contract: `design/ventModelContract.md`); until then the law exists twice — inline here and as the tested reference `vent_model_stepped` |
...
| T17 | `window_pos` | **Window position** (Phase 2, 2026-09-10; in `main` since 2.8.0): polls the M3 wire encoder at a **derived** interval (`travel_m3`/150) while **M3** travels, and reads it every 30 s at rest. A presence gate selects POSITION or TIMED control and re-probes an absent sensor every 30 s. **Since 2.9.0 (gh#73) only if `motor/wpos_fitted_m3` = 1** (default 0): not fitted, T17 sends nothing and publishes TIMED with reason `NOT_FITTED`. **Since 2.9.1 (gh#72) the two fault checks judge each DRIVE**, found through T2's `t2_get_drive()` counter, so a reversal is two. Priority **4** — deliberately below T2/T3 (6) and T4/T5/T6 (5): a Modbus transaction can hold the bus ~215 ms and must never sit in front of a wind override. **Only possible because 2.4.1 shipped the bus mutex (gh#49)** — a second bus caller was a data race before that. Observe-only in 2.8.0: publishes a snapshot, the control mode and two fault checks (§12.4 rules 1 and 2), and changes no control |

Also: a low-priority `heartbeat_task` is spawned at `main.cpp:1599` as a serial-only liveness indicator — not part of T1-T17 numbering.
...
| `window_pos/` | T17 — M3 wire-encoder polling + derived config; logs position (SENSOR_HR ch3) and device events (ALARM ch6). Driver lives in `drivers/windowPos/` |
