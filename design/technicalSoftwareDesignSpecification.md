...
- Page 5: motor (window) states — row 0: `M1    M2    M3  `; row 1: four-character state per channel (`OPEN` / `CLOS` / `MOV>` / `MOV<` / `UNK `); state read via `t2_get_window_states()` (FR-UI04). **rc.1.5.0+ (gh#29):** `#` enters `UI_MOTOR_PICK` (1=M1, 2=M2, 3=M3, *=back) → `UI_MOTOR_ACTION` (1=Open, 2=Close, *=back). **Admin PIN only.** **rc.1.5.2+ semantics**: menu entry takes a STANDBY hold, `dm_standby_hold(DM_STANDBY_HOLD_LCD, LOG_BY_ADMIN, 1)` (2.8.0, gh#65). It takes the hold if STANDBY is off, joins it if another session already holds it (a web teach), and leaves an operator's own STANDBY alone; whether this session holds is tracked in `s_manual_set_standby_on_entry`. Before 2.8.0 the entry called `dm_set_standby_ex(true, …)`, which persisted the STANDBY while that flag lived only in RAM, so a reboot during the session stranded the unit in STANDBY. T6 stays gated by the standard `EG1_BIT_STANDBY` for the entire admin PIN session — not just while the admin is keypressing in the menu. **Menu does NOT auto-dismiss on idle.** **`*=back` does NOT clear STANDBY** — it only navigates back to the auto-rotating status screens; STANDBY persists. The hold is released when the admin session ends via `session_close()` (5-minute timeout from last keypress or explicit logout). If it was the last hold, STANDBY clears, T2's dwell debt is dropped (`T2_NOTIFY_CLEAR_DWELL`) and the windows recalibrate (since 2026-09-10). A hold is never written to NVS, so a reboot ends it together with the session. Re-entering the menu within the same session preserves the flag (`true` stays `true`). If the operator had switched STANDBY on before the menu was entered, the flag stays false and the session-end leaves STANDBY untouched. Verified on hardware by `bin/at_lcd_standby.py`. Actions post `window_cmd_t` to Q1 with `source = SRC_OPERATOR_MANUAL`; T2 dwell-timer check is bypassed for that source. Safety gates remain authoritative: `WIND_OVERRIDE` blocks OPEN (CLOSE accepted); `MOTOR_ALARM` and `CALIBRATING` block all manual commands.
