...
| IT-EL-017 | IT | Proactive free-space guard: oldest file deleted when SD free < 4 MB (above floor); logging suspended at the 5-file floor | Fill SD card to < 4 MB free with > 5 log files present; observe T9 serial | Serial: `SD low space: deleted oldest (N files remaining)` when count > 5; if already at 5 files: `SD low space at retention floor — suspending`; `GET /api/sd/status` shows `mounted: false` at floor |
...
| IT-EL-019 | IT | Multi-file log upload finds the **newest closed** file when the card holds > 21 CSV files (gh#42 scan-buffer-truncation regression) | Stage > 21 closed CSV files on the SD (or temporarily lower `SD_MAX_FILES`); let the active file rotate past 1 MB; observe the upload + the SD audit rows | Just-closed file uploads — audit row `initiator=WEB, value_a=1, value_b=1`; the daily `value_a=0, value_b=2` ("no closed file") skip does **not** fire while closed files exist. Guards the `char list[512]` → `SD_LIST_BUF_LEN` sizing in `event_logger_next_pending()` / `event_logger_newest_closed()` (gh#42, fixed 2.2.15) |
