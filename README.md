...
| **`hardware/`** | KiCad project for the PCB (`GreenhouseControllerPCB/`), fabrication outputs (`fabrication/` — Gerbers, BOM, pick-and-place), Modbus RS485 cable junction design (`ModBusRS485CableJunction/`), and bench-test setup (`Testing/`). | [`hardware/GreenhouseControllerPCB/README.md`](hardware/GreenhouseControllerPCB/README.md), [`hardware/fabrication/README.md`](hardware/fabrication/README.md) |
| **`design/`** | Functional & technical design documents that drove the implementation: requirements, architecture, LCD GUI design, default-settings rationale, FreeRTOS task analysis, web-GUI mouseover specs, version-control discussions, etc. | [`design/functionalRequirementsSpecification.md`](design/functionalRequirementsSpecification.md), [`design/technicalSoftwareDesignSpecification.md`](design/technicalSoftwareDesignSpecification.md), [`design/defalutSettingsMotivation.md`](design/defalutSettingsMotivation.md) |
...
| [Local UI — LCD / keypad](design/technicalSoftwareDesignSpecification.md) | Screen layouts, menu structure, FSM, user-interaction patterns — §5.5 of the software TSDS. *(The standalone `LCD_GUI_Design.md` was retired at 1.16.3; an unversioned `.docx` of the original remains in `design/` but is not tracked.)* |
...
| [Firmware Implementation Results](Archive/firmwareImplementationResults.md) | Phased implementation results from the original build-out. *(Superseded — the companion `firmwareImplementationPlan.md` was retired once the phases completed; current work is tracked in `changelog.md` and GitHub issues.)* |
