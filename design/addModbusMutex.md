...
**It became real, and it is fixed — 2026-09-16 (gh#69, closed).** The condition this note rested on, *T5 is the sole caller*, stopped being true on `ropeSensor` when T17 began polling the encoder, and nobody came back here. FDA4 then panicked **twice in a row** after an OTA reboot: `LoadProhibited` at `0x14` inside `uart_get_buffered_data_len()`, called from T17's stroke poll (`windowpos_read` → `modbus_transaction`). T2's boot recalibration was driving M3, so T17 was polling every 100 ms, and T5's entry re-init — 8 s after its start — deleted the driver under T17's transaction. The third boot survived only because T17's presence probe happened to succeed later than the re-init. **Fix:** `modbus_init()` now takes the bus mutex on a re-init (waiting up to 2 s, against ~235 ms worst-case hold) and, if it cannot get it, leaves the installed driver alone and logs an error; the first call has nobody to wait for. **Verified fail-first**, the protocol of §4.3c: `python bin/at_modbus_reinit.py --host <ip>` asks a bench build to call `modbus_init()` hundreds of times from a task at T5's priority, optionally while a companion task keeps a read in flight, and the firmware reports `reinit_locked` so a result cannot be read off the wrong build. With the lock compiled out (`MODBUS_FAILFIRST_UNLOCKED_REINIT`, one line in `modbus_rtu.cpp`) **five runs crashed five times** — two interrupt-watchdog lockups and three panics, two of them with the exact field signature (T17, `LoadProhibited` at `0x14` via `windowpos_read`). With the lock, **500, 2 000 and 3 000 re-inits all passed**: nothing skipped, no encoder failure, free heap back to within 256 bytes. Four natural boots of the fixed build also ran the original scenario without a panic. Results in full on gh#69. **The lesson is about the note, not the code:** a hazard recorded as *"safe today because X"* needs X watched, and adding a second bus caller was exactly the change that should have sent someone back here.


### 6.2 Found and fixed 2026-09-16 — the DE/RE release was timed by the task (gh#70, closed)

The lock makes a transaction exclusive; it does not make it punctual. `modbus_transaction()` drops DE/RE after `uart_wait_tx_done()` and a 2 ms guard, in task context, and the wire encoder answers ~4.5–5 ms after the request. A flash erase or write stalls both cores and holds back the (non-IRAM) TX-done interrupt, so during an OTA the release came up to **665 ms** late and the reply was lost. Measured with the driver's DE-release counters and `bin/at_modbus_ota.py`: every encoder failure followed a late release, in all six runs. **Fix:** UART1 runs in `UART_MODE_RS485_HALF_DUPLEX` with its RTS output on the DE/RE pin, so ESP-IDF raises the line when a request enters the TX FIFO and drops it in the TX-done interrupt; `CONFIG_UART_ISR_IN_IRAM=y` keeps that interrupt running while the flash is written; and the receive loop judges its deadline only when no byte is waiting, so a stalled task still collects a buffered reply. The unit reports `de_ctrl: "uart+iram"`. **Verified fail-first on FDA4** with `bin/at_modbus_ota.py`: the fixed build passed baseline 2 166 reads / 0 failures (was ~1 in 540), firmware upload 686 / 0 (was 16 and 13), asset extraction 112 / 0 (was 7 and 9), while the same code with `MODBUS_FAILFIRST_TASK_DE` failed the firmware phase with 17 encoder failures, every one after a late release (worst 277 ms) — **with the UART interrupt already in IRAM**, so that setting alone does not fix it; the hand-over does. Re-init stress (gh#69), the teach test and T5's reads also pass on the fixed build. **The 12F0 loopback suite passes 12/12 on the fix** (`pio test -e lolin_s3_loopback -d drivers/modBus`, run twice, 2026-09-16), and its DE/RE tests were shown to fail on two deliberately broken builds: with RTS never routed to the DE/RE pin, HW-MB-002 failed; with RS485 mode unset and RTS held high, HW-MB-001 and HW-MB-006 failed. **Not verified here:** a scope trace of the DE/RE pin.
