...
| FR-LG06 | The system **shall** retain event log entries persistently on an SD card using a rotating-file strategy. Each file **shall** be capped at a maximum size (default 512 KB); when the file reaches the cap, the system **shall** close it and open a new file timestamped with the rotation moment. A bounded number of rotated files **shall** be retained on the card (default 10), the oldest file being deleted automatically when the cap is exceeded. The rotation cap **shall** be sized such that worst-case activity at the minimum poll interval produces no more than a few rotations per day, keeping the on-card retention measured in days even under load. | Must |
...
| FR-LG09 | The log **shall** include a sensor-value snapshot (temperature, humidity, wind speed, wind direction) on every sensor poll cycle. The snapshot interval therefore equals the poll interval (FR-S03); no separate snapshot interval is configurable. Temperature **shall** be recorded at 0.1 °C resolution; humidity, wind speed, and wind direction at the resolution provided by the corresponding sensor. | Must |
