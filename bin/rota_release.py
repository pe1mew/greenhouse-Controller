...
Usage:
    python bin/rota_release.py publish 2.2.12
    python bin/rota_release.py publish 2.2.12 --dry-run
...
def build_parser():
    ...
    # (e.g. `status --local DIR`, `publish 2.2.12 --dry-run`).
    common = argparse.ArgumentParser(add_help=False)
    common.add_argument("--dry-run", action="store_true",
                        help="print planned actions; make no remote/local changes")
    common.add_argument("--ssh", help="SSH target (alias or user@host); or ROTA_SSH")
    common.add_argument("--store", help="ota-store path on the VPS (default %s)" % DEFAULT_STORE)
    common.add_argument("--local", help="use a LOCAL ota-store dir instead of ssh")
    ...
    sp = sub.add_parser("publish", parents=[common],
                        help="publish a built release and point soak")
