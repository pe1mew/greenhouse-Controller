...
| 3.3 | **`/api/ota/config` endpoint + GUI group** — admin-only transaction endpoint (validate-then-write, empty-secret/cert = keep); System-tab group: enable, interval, URL, secret, night-window, PEM upload, read-only last-check line; farmer allowlist excludes all keys. **Status: endpoint DONE 2026-07-13** — GET/POST `/api/ota/config` (admin-only, validate-then-write, empty secret/cert = keep, secret never echoed R-A09), POST notifies T16 to re-check now (R-F04); `max_uri_handlers` bumped. **GUI System-tab group DONE 2026-07-13** — admin-only group in `data/index.html` + `data/app.js` (enable, check interval, URL, secret [blank=keep], night window, PEM upload, read-only "Last check" line, "Check now"); loads on System-tab open. Verified in a static-serve browser harness: panel populates from `/api/ota/config`, POST omits blank secret/cert (keep) and includes filled ones, last-check line renders from `/api/ota/check`. Farmers never see the System tab. | R-F02, R-F03, R-F05, R-A03, R-A09 | TC-14 |
...
| 3.10 | **Docs same changeset** — OTAimplementation.md addendum, beheerder manual (config surface), boer manual one-liner, changelog `## [2.2.0]`, `bin/2.2.0/release-notes.md`. **Status: DONE 2026-07-13** — changelog `## [2.2.0]` ROTA entry; `OTAimplementation.md` §12 addendum (+ §9 reconciled); `logparser.py` decodes audit `value_a=22/23/24`; `beheerderHandleiding.md` §10.5 ROTA config section; `boerHandleiding.md` §12.2 "Update pending" one-liner. **Remaining:** `bin/<ver>/release-notes.md` (write at release-cut time). | R-F06 | TC-14 |
...
| 5.8 | Heap + control cadence during a full update. **Criterion restated 2026-09-20 (gh#81), operator's decision:** judge the **steady-state internal free heap and largest free block at a stated uptime**, not `heap_min_kb`. The old wording, *floor ≥ 20 KB*, reads a since-boot extreme that can only fall: it is the worst single moment across every TLS handshake the unit has ever made, so on a long-running unit it reports history, not headroom. Measured on 2344 (2.10.1-bench, 2026-09-20): steady state **63-71 KB free, 24-31 KB largest**; each T14 status POST dips to 39-50 KB free / 13 KB largest for under a second, and the floor walks down one step per POST (33 KB at 24 s after boot, 13 KB at 35 min, 9-11 KB over a day) with no leak -- free returns to ~70 KB after every POST. `heap_min_kb` stays published and stays in the release comparison as a **diagnostic**, read at a stated uptime. Tracked as **gh#40** and **gh#81**. | TC-09 |
