...
| **Soak ≥ 12 h with scripted strokes** | **PASS twice on 9c53be7** (bench image `de7d9228…`, 2344). **First run** 2026-09-21 17:55 → 2026-09-22 06:05: 12.17 h, 24 judged strokes, all confirmed, `stall_faults` / `early_stops` / `rejected_rate` / `err_comm` / `not_reached` 0, `mode_changes` 0, gate settled, no reboot; all 12 scripted sessions ran. **Re-soak after the nine test pushes**, 2026-09-22 22:50 → 2026-09-23 11:00: **12.17 h, 24 judged strokes, same zeros**, and still **PASS at 22.03 h with 26 judged strokes** — the extra two are a drive T6 made by itself, confirmed like the rest. Measured traverse 11.9-12.4 s against `travel_m3` 13. **Heap, sampled every minute for 22 h (1 320 samples, `heap_watch.py`):** free 41-67 KB, largest block 20-29 KB, steady state 66/29 — the TLS status POST dip never went below 41 KB free. The since-boot floor stepped 23 → 16 → 15 KB in the first 12.5 h and has not moved since; neither step was near a ROTA check. *The 1 KB floor seen at the end of the first soak has not recurred and is still unexplained: deliberately reproducing that day's diagnostic load — a 2 s status poll for 3 min plus admin diag reads and the log listing — did not move the floor at all.* Two earlier runs are superseded: 12.04 h on 2026-09-20 (the stranded-M3 image) and 2.06 h on 045a39c |
