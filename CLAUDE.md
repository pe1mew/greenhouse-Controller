...
Production deployment at Herenboeren Willemshoeve (Soest). Field units in active service (as of 2026-09-07): **5C88** (production, ROTA `mainstream` channel) and **FDA4** (ROTA `soak` channel — **the** development and soak unit). **2344 is stowed as a cold spare** — not powered, not on the network; it is no longer a soak target and `ota_push.py`'s built-in default still points at it (see the OTA-push row below). FDA4 is now a **fully equipped controller**, not the bare board it was until 2026-09: sensors, SD and relays are live (verified `eg1 = 0x0`, SD mounted, plausible T/RH/wind). Current addresses, `devices.json` MAC keys, and access routes live in user-global memory.
...
| OTA-pushing to a unit | `python bin/ota_push.py <bin-path> --host <ip>` — current unit IPs in user-global memory. **Always pass `--host`.** The built-in `HOST_DEFAULT` is `192.168.20.160` = **2344, which is stowed and off the network**, so an unqualified push targets a dead address. FDA4 = `192.168.20.169`. Default PIN `12345678` (admin), `1234` (farmer) |
...
5. OTA push to soak **FDA4**: `python bin/ota_push.py bin/X.Y.Z/greenhouse-controller-X.Y.Z.bin --host 192.168.20.169` (the `--host` is not optional — the built-in default is the stowed 2344)
