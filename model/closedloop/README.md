...
| The law over time | `graded_walkthrough.py` | What `graded` does to M1, M2 and M3 as the temperature and the humidity move: the two ladders, M3's aperture map, what each `cr_priority` makes of a humidity vote, and two synthetic days driven through the law and the emulated firmware, each against mode 1. Prints the tables in [`gradedCandidate.md`](gradedCandidate.md), "How it behaves over time" |
| Humidity audit | `humidity_audit.py` | What the humidity branch asked for over 5C88's whole history and what the conflict rule did with it, plus two rules the firmware does not have: a close guard on the RH branch and a floor under a humidity-only open. Joins the outdoor sensor to ask whether venting would have removed water. Prints the tables in [`humidityControl.md`](humidityControl.md) |
...
The emulation follows 2.12.0 as built (045a39c, and 9c53be7's three fixes to what the law is told): T2's `ch_start_target()` and `ch_target_tick()` with the overrun lead, its takes and its CLOSE_ALL sweep, T17's settle read, and T6's `plan_target()`, `apply_model_output()`, `post_target()` and `judge_m3_target()`. `test_linear_m3.py` checks it against those rules.
...
- **What the law gets:**
  ...
  - Its last target and how it ended, **as T6 keeps them**. The target is recorded when T6 posts it, with T2's count of **takes** then: drives the law did not command, from T3, the operator or a sweep. It is judged once M3 is at rest: **ABORTED** if the count moved, else DONE within the deadband of the target, else FAIL_TIMEOUT, which includes a command T2 deferred and never started. A repeat of the outstanding target keeps the count it went out with, so a take while M3 moves is not folded into the baseline.
  - The time since its last drive ENDED, also while another one runs. None since boot is `UINT32_MAX`, long ago, in the firmware since 9c53be7 and here.
...
- **T2:**
  ...
  - **A reversal mid-stroke** is deferred for T6 (gh#48), and the stroke keeps its target: it stops at its own aim, and T6 asks again. Until 9c53be7 the deferral disarmed the target, and the stroke ran on to the end switch. Any command that acts still disarms, so a safety close is never stopped short.
  - **Takes** (`t2_get_taken()`): a command from anyone but T6 that starts the window, reverses it, or turns a targeted stroke into a full one, and every sweep. A command that finds the window already where it sends it takes nothing.
- **The sweep** (`calib_close_all()`) runs at a reboot unless all three windows were closed (gh#18's shortcut, which also forgets the dwells), and at STANDBY's end in today's firmware. 5C88 never swept at STANDBY's end: its log has no sweep after any of its six exits. Every window is driven closed at once, for its own travel, each counted as a take. Each end arms the close dwell, `ch_dwell_ms()`, so `min_intv_m3` for M3 in mode 2, and ends a move. T2 reads no command until the slowest window ends. Commands wait in Q1, 8 deep, and go out when it returns. Today's firmware pauses T6 meanwhile (gh#79). 5C88's queued T6's commands instead, which is why its M1 and M2 opened the second the sweep after its 07-17 reboot returned, and `test_linear_m3.py` reproduces that. A reboot takes effect at the first 30 s sample after it.
- **The report adds:**
  - what became of the targets: moved, dropped, deferred, timed out, taken over;
  - T6's judgements (DONE, FAIL_TIMEOUT, ABORTED), T2's takes of M3, and the sweeps;
