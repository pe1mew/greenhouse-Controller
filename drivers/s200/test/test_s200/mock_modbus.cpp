 ...
 * @brief Modbus RTU stubs implementation for the native S200 unit-test build.
 *
...
static const int BANK_SIZE = 512;    /* register address space covered (0x0000–0x01FF) */
...
/* Last FC04 (input register) read tracking */
static uint8_t   s_last_read_addr;
static uint16_t  s_last_read_reg;
static uint8_t   s_last_read_count;

/* Last FC16 write tracking */
...
uint8_t  mock_modbus_get_last_read_addr(void)  { return s_last_read_addr; }
uint16_t mock_modbus_get_last_read_reg(void)   { return s_last_read_reg; }
uint8_t  mock_modbus_get_last_read_count(void) { return s_last_read_count; }
...
/* FC04 — used by s200.cpp for all sensor reads */
modbus_status_t modbus_read_input_registers(uint8_t  device_addr,
                                             uint16_t start_reg,
                                             uint8_t  count,
                                             uint16_t *out)
{
    s_last_read_addr  = device_addr;
    s_last_read_reg   = start_reg;
...
/* FC03 — not used by s200.cpp but provided to satisfy the linker */
...
/* FC16 — not used by s200.cpp in normal operation */
