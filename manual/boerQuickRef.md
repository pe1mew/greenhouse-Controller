...
| `Mode: STANDBY` | Operator-pauze — ramen blijven staan; vergeet niet om uit te zetten als je klaar bent (zie `boerHandleiding §10.4`) |
...
>
> Verlaat je Standby (kies "Auto"), dan loopt ook een kalibratie van ~3 min — dat is normaal, niet ingrijpen.
...
Toets **`D`** = vanaf een statusscherm: direct naar volgend scherm; **vanuit elk menu / PIN / bewerk-scherm: direct terug naar de auto-rotatie**. Toets **`#`** = direct naar het bijhorende menu (werkt op T/RH, Wind, Mode/Sess voor Standby, WiFi, Datum/tijd; op Raamposities alleen voor de Beheerder). Sensor-uitval toont: `** SENSOR FAULT` op rij 2 + LCD kleut rood → bel beheerder.
...
| Standby aan/uit zetten | LCD scherm 3 → `#` → Boer-PIN → `1=Auto / 2=Stby`. Of web Climate-tab → Mode-keuzelijst → Apply. Volledige uitleg: boerHandleiding §10.4 |
| Controller blijft in `Mode: STANDBY` na opstart | Standby was bewust aan toen de stroom uitviel — zet hem uit zoals hierboven, ramen kalibreren ~3 min, daarna `Mode: AUTO` |
| Gele **Standby** badge in webinterface (Alarms-tegel) | Operator-pauze actief; zet uit via Climate-tab Mode-keuzelijst of LCD Scherm 3 # (rc.1.5.1+) |
