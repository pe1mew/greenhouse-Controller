...
- **2026-09-16** — the board PANICs ~10 s after boot, twice, then runs fine (T5's entry `modbus_init()` deleted the UART under a T17 read — a hazard logged 9 days earlier as "safe while T5 is the only caller"; coredump says LoadProhibited in `uart_get_buffered_data_len`)
...
**Fix.** A re-init takes the bus mutex, waiting up to 2 s, and if it cannot get it leaves the installed driver alone and logs an error. Verified on FDA4 over both reboots of the next push, each the crash scenario (M3 recalibrating, T17's gate open before T5's re-init), with no panic.
...
**Where it lives.** `drivers/modBus/src/modbus_rtu.cpp` (`modbus_init`), `drivers/modBus/src/modbus_rtu.h` (the `@note`), `firmware/src/main.cpp` (init comment), `design/addModbusMutex.md` §6.1.
