...
### 5.3b Operator-initiated Standby Mode

Distinct from the safety-driven Motor Alarm and Wind Override states, **Standby** is an operator-initiated pause of autonomous climate control — used for scheduled maintenance, manual operations, and any period during which the operator wants the controller to leave the windows alone.

| ID | Requirement | MoSCoW |
|----|-------------|--------|
| FR-MD01 | The operator (Farmer or Administrator) **shall** be able to toggle the system between **Automatic** and **Standby** operating modes from both the local controller (LCD keypad) and the web user interface. | Must |
| FR-MD02 | In **Standby** mode the climate-control loop **shall** suspend issuing window commands. Windows **shall** remain in their current positions on STANDBY entry (no forced movement). | Must |
| FR-MD03 | When the system transitions out of **Standby** the controller **shall** automatically run a CLOSE_ALL re-calibration cycle (same procedure as boot-time calibration) before normal climate control resumes. | Must |
| FR-MD04 | Safety overrides **shall** retain authority over Standby: Wind Safety override and Motor Alarm both win the priority chain over Standby (an admin who put the unit in Standby cannot prevent the controller from defending the greenhouse against a gust or a stuck motor). | Must |
| FR-MD05 | The Standby state **shall** be persisted to non-volatile storage so that the controller returns to the operator's last-selected mode after a power cycle. A brownout during a deliberate Standby **shall not** silently re-enable climate control on reboot. | Must |
| FR-MD06 | Every Standby entry and exit **shall** be logged with a timestamp, the operator role (Farmer / Administrator) that performed the toggle, and the surface (local LCD or web GUI) on which it was performed. | Must |
| FR-MD07 | The web GUI Standby control **shall** be disabled (greyed out, no commit accepted) whenever Wind Safety override, Motor Alarm, or boot-time calibration is currently active. The reason is communicated by the visible mode badge. | Should |

### 5.3c Administrator Manual Motor Control (LCD)

Distinct from setpoint-driven automatic ventilation and from the safety-driven CLOSE_ALL behaviour, this section governs the Administrator's ability to directly open or close individual window channels from the physical controller — used for commissioning, maintenance with gloved hands, network outages, emergency overrides, and demonstrations.

| ID | Requirement | MoSCoW |
|----|-------------|--------|
| FR-MM01 | The Administrator **shall** be able to open or close each window channel (M1 / M2 / M3) individually from the controller's LCD + keypad surface. | Must |
| FR-MM02 | Manual motor control via the LCD **shall** require an active Administrator session. Farmers **shall not** be able to issue manual motor commands; the design philosophy is that farmers are supported by the controller's autonomous logic, not by bypassing it. | Must |
| FR-MM03 | Entering the manual-control menu **shall** auto-enter Standby mode (gh#28) for the duration of the **Administrator's PIN session**, so that the autonomous climate-control loop stays paused not only while the admin is keypressing but for a configurable respect-window after they leave the menu. The menu **shall** stay on screen until either an explicit back-key exit or the Administrator session timeout (`cfg.session_timeout_min`, default 5 minutes); pressing back **does not** end Standby — it only navigates back to the auto-rotating status screens. Standby persists across menu re-entries within the same session. The menu auto-set Standby **shall** be tracked so that the matching auto-clear at session-end fires only if THIS menu entry set it; if Standby was already on when the menu was entered, the session-end **shall** leave Standby untouched. *(rc.1.5.2+ — rc.1.5.1 cleared Standby on `*=back`, which let T6 resume on the next sensor poll (~30 s) and overwrite the admin's manual positions; physical testing confirmed this was undesired. rc.1.5.0 used a separate short-lived EG1 bit with a 10 s idle dismiss with the same root cause.)* | Must |
| FR-MM04 | Manual commands **shall** bypass the anti-thrash dwell timers that gate the autonomous loop (the Administrator's deliberate choice overrides anti-thrash protection). Dwell timers **shall** apply normally to autonomous commands issued after the Administrator exits the menu. | Must |
...
| FR-MM07 | When the Administrator's PIN session ends (5-minute timeout from last keypress, or explicit logout), the manual-menu-initiated Standby **shall** clear *without* a CLOSE_ALL recalibration — the admin's manual per-channel positions **shall** be preserved and the controller's next decision **shall** be taken based on the actual current state. This is deliberately distinct from the Scherm 3 / web GUI Standby exits (FR-MD03), which DO recalibrate: those surfaces are explicit operator pauses where a clean baseline is appropriate; the manual-menu Standby is a respect-window around a sequence of deliberate per-channel commands. *(rc.1.5.2+ — implemented via `dm_set_standby_ex(..., recalibrate_on_clear=false)` called from `session_close()` when the `s_manual_set_standby_on_entry` flag is set; locked decision 2026-05-26 follow-up "Respect window, T6 acts after the session timeout from admin at LCD.")* | Must |
...
> **Note:** The current hardware design uses green (power, heartbeat) and amber (relay activity) discrete LEDs. In addition, the on-board WS2812 RGB LED (see §5.9.2) provides system-level status indication using a separate colour convention specific to that LED.
...
| FR-UI13 | The controller **shall** include a green heartbeat LED driven by the firmware. The heartbeat LED **shall** blink at 1 Hz during normal operation, blink at 4 Hz during start-up initialisation, remain steady-on if the firmware has stopped (watchdog not yet fired), and be off if the MCU is not running. The heartbeat LED follows the green-means-operation-is-OK convention of §5.9.1 — a steady blink confirms the firmware is alive and the controller is operating normally. | Must |
...
> **Status:** MQTT integration is a *Could-have* feature. The system architecture leaves room for it (T12 task slot reserved; `LOG_BY_MQTT` initiator code allocated; web configuration page accommodates broker settings) but the feature is not currently active in the running firmware. Activation is conditional on operator demand. The controller operates fully without it (FR-MQ05).

| ID | Requirement | MoSCoW |
|----|-------------|--------|
| FR-MQ01 | The controller **could** connect to a user-configured MQTT broker when WiFi is available. | Could |
| FR-MQ02 | The controller **could** publish current temperature, humidity, window states, and alarm status to the MQTT broker at regular intervals. | Could |
| FR-MQ03 | The controller **could** accept window commands and setpoint changes via subscribed MQTT topics. | Could |
| FR-MQ04 | If MQTT is provided, the broker address, port, and credentials (username and password) **shall** be configurable by the administrator via the web configuration interface. | Could |
| FR-MQ05 | MQTT connectivity **shall** be optional; the controller **shall** operate fully without an MQTT broker. | Must |

> **Design decision:** MQTT configuration (broker address, port, and credentials), if provided, is accessible only through the web configuration interface; it is not available in the local keyboard menu.
...
| FR-LG05 | The log **shall** be retrievable via the web interface (when WiFi is available) and by removing the SD card. | Should |
| FR-LG06 | The system **shall** retain event log entries persistently on an SD card using a rotating-file strategy. Each file **shall** be capped at a maximum size (default 1 024 KB); when the file reaches the cap, the system **shall** close it and open a new file timestamped with the rotation moment. A bounded number of rotated files **shall** be retained on the card (default 30), the oldest file being deleted automatically when the cap is exceeded. The rotation cap **shall** be sized such that worst-case activity at the minimum poll interval keeps the on-card retention measured in weeks-to-months even under load. | Must |
| FR-LG07 | The SD card **shall** be the primary persistent log store. The system **shall not** maintain a separate internal-flash ring buffer for event log data; persistent retention is via the SD card only. | Must |
| FR-LG08 | If the SD card is absent, removed, or returns a write failure, the system **shall** suspend on-card logging cleanly (closing the current file before the next write attempt) and **shall** surface the SD-unavailable condition to the operator via the local UI and the web GUI. Climate-critical operation **shall** continue unaffected; on SD re-insertion the system **shall** resume logging in a fresh file timestamped with the resumption moment. | Must |
| FR-LG09 | The log **shall** include a sensor-value snapshot on every sensor poll cycle, recorded as three companion rows sharing the same timestamp: (1) indoor temperature + relative humidity, (2) wind speed + wind direction, (3) per-channel window state plus the active safety flags (wind override, motor alarm, calibrating). The snapshot interval therefore equals the poll interval (FR-S03); no separate snapshot interval is configurable. Temperature **shall** be recorded at 0.1 °C resolution; humidity, wind speed, and wind direction at the resolution provided by the corresponding sensor; window state at the resolution of the public state enum (CLOSED / MOVING_OPEN / OPEN / MOVING_CLOSE). | Must |
| FR-LG10 | The system **shall** capture a coredump to a dedicated non-volatile partition when the firmware panics. The presence of an unread coredump **shall** be surfaced to the operator via a flag in the local web GUI alarms area and as a tile in the diagnostics page. An administrator **shall** be able to download the coredump for offline analysis and to erase it from the device once analysis is complete. Both actions **shall** be audit-logged with operator attribution. | Should |
| FR-LG11 | At each boot the system **shall** log the previous-boot reset reason (e.g. POWERON, SW-restart, PANIC, WDT, BROWNOUT) so that every fresh log file opens with a verdict on the immediately preceding shutdown. | Should |
| FR-LG12 | The log **shall** record the day's sunrise and sunset times (local-time minutes from midnight) whenever the cached values change — in steady-state operation this means once per local-midnight rollover, plus once at every boot and once per operator coordinate edit. The row makes per-day night-shading in the analysis pipeline self-sufficient: a historical log file alone is enough to reconstruct that day's day/night geometry without needing a separate live lookup. | Should |

### 5.15 Fault Isolation Between Subsystems (Bulkhead Policy)

> **Rationale.** The controller has a primary mission-critical subsystem (sensor poll, climate control, relay actuation, motor alarm handling) and a set of secondary best-effort subsystems (WiFi client, web GUI, MQTT, status-website reporting, log upload). A fault inside a secondary subsystem — for example a hang inside the TLS handshake of an HTTPS POST, or a slow memory leak inside a third-party library — must not be allowed to disrupt or delay the primary subsystem. This section codifies the contract between the two layers.

> **Status:** The advanced bulkhead protections in this section (persistent circuit breaker FR-BK03, supervisor task FR-BK04, and the corresponding operator surfacing FR-BK06) are *deferred*. They were specified to defend against a heap-fragmentation failure mode in the secondary network stack that has since been eliminated structurally by a different mitigation (smaller TLS buffers, persistent TLS session reuse, and direct mbedtls configuration). If the long-running soak test confirms that the simplified stack is sufficient on its own, these requirements may be withdrawn from the specification at the next revision. The other requirements in this section (FR-BK01 priorities, FR-BK02 timeouts, FR-BK05 window-state persistence, FR-BK07 heap instrumentation, FR-BK08 watchdog-friendly chunking) remain in force as they do not depend on the deferred items.
...
| FR-BK03 | *(Deferred — may be withdrawn pending soak outcome.)* Repeated failures of any secondary outbound operation (status POST, log upload, MQTT publish) **shall be** throttled by a persistent circuit breaker: after N consecutive failures, further attempts **shall be** suppressed for an escalating duration (60 s → 5 min → 30 min → 1 h cap) until a successful attempt regresses the escalation phase. The breaker state **shall** survive a reboot via non-volatile storage so that a power-loss during a failure burst does not reset the throttling. | Should |
| FR-BK04 | *(Deferred — may be withdrawn pending soak outcome.)* The controller **shall be** capable of running a supervisor task that monitors the primary network-reporting task for liveness (heartbeat counter) and for resource consumption (cumulative heap drop). If the monitored task is judged stuck or leaking, the supervisor **shall** either respawn it (cleanly closing any persistent network sockets first) or, if respawning is itself failing, issue a *planned reboot*. The reset reason of a planned reboot **shall** be distinguishable in the boot log from a panic or watchdog reset. | Should |
| FR-BK05 | Terminal window-position state (per channel, fully OPEN or fully CLOSED) **shall** be persisted to non-volatile storage on every transition. At boot, if all three channels were persisted as CLOSED and no motor-alarm signal is asserted, the boot-time CLOSE_ALL calibration **may** be skipped, reducing recovery time after a planned reboot from ~171 s (full M3 travel) to ~2 s. The pre-energise persist of `UNKNOWN` to non-volatile storage **shall** precede every energise of a relay so a power-loss mid-travel is recoverable. | Should |
| FR-BK06 | *(Deferred — may be withdrawn pending soak outcome.)* Whenever the persistent circuit breaker (FR-BK03) is in an open state, the controller **shall be** capable of surfacing this fact to the operator: as an on-LCD indicator (compact badge on the network status screen), as a flag in the canonical status JSON payload of the local web GUI, and as a written record in the event log when the breaker phase transitions. The breaker state **shall not** affect the primary climate control. | Should |
...
| TR-NW04 | Data exchanged via the web interface **shall** be protected via shared-secret authentication and confinement to same-LAN traffic. | Could |
