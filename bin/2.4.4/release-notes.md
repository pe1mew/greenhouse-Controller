...
**The soak channel deliberately still offers 2.4.1** while FDA4 runs 2.4.4. 2.4.2 through 2.4.4 were push-OTA'd direct and not published to ROTA — an intentional choice for rapid dev iteration on a bench-reachable unit, confirmed by the operator 2026-09-10. `up_to_date` is anti-downgrade refusing to go backwards. The accepted consequences: a reflashed or replaced FDA4 would be pulled back to 2.4.1, and 5C88 on `mainstream` stays on 2.3.1 until something is deliberately promoted. Publish to `soak` when a release is a genuine soak candidate, not per iteration.
