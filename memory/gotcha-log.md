...
**Root cause (gh#70).** `modbus_transaction()` drops DE/RE in task context: `uart_wait_tx_done()`, a 2 ms guard, then `gpio_set_rs485_direction(false)`. A flash erase or write stalls both cores and — with `CONFIG_UART_ISR_IN_IRAM` off — holds back the TX-done interrupt, so the release comes late. The encoder answers ~4.5–5 ms after the request, leaving ~2 ms of margin: a late release keeps the transceiver driving into the reply, which arrives as 0 of 7 bytes or 6 of 7 (CRC). **Measured with new DE-release counters:** every encoder failure followed a late release and every late release was followed by a failure — firmware upload 16/16 and 13/13 (worst **665 ms**), asset extraction 7/7 and 9/9 (worst 18.6 ms), and even a quiet baseline 1 in ~540 (4.7 ms, scheduling jitter). T5 is exposed too: a 665 ms stall beats every slave, and two T5 failures are a wind-sensor fault, which is a WIND OVERRIDE — during a ROTA update on `main`, not only on the rig.
...
**Root cause.** `modbus_init()` deletes and reinstalls the UART driver, and T5 calls it again at task entry, after an 8 s grace. It took no lock. `design/addModbusMutex.md` §6.1 had logged exactly this on 2026-09-07 — *"a use-after-delete … safe today because T5 is the sole caller"* — and `main.cpp` said the same. On `ropeSensor` T17 is a second caller. T2's boot recalibration was driving M3 (M1 had been left open, so NVS was not all-CLOSED), so T17 polled every 100 ms from the moment its presence probe succeeded, and T5's re-init deleted the driver under one of those reads. Whether it crashes depends on whether T17's probe beats T5's re-init: the third boot's probe was late, so it survived. **Earlier boot recalibrations on this branch had not crashed for the same reason** — the early-boot bus is often dead (gotcha 2026-09-07), the probe fails, and the gate opens 30 s later.

**Fix (gh#69, closed).** A re-init takes the bus mutex, waiting up to 2 s, and if it cannot get it leaves the installed driver alone and logs an error. **Verified fail-first** with `python bin/at_modbus_reinit.py`, which makes a bench build call `modbus_init()` hundreds of times under bus traffic: with the lock compiled out (`MODBUS_FAILFIRST_UNLOCKED_REINIT`) 5 runs gave 5 crashes, two of them the exact field signature; with it, 500 / 2 000 / 3 000 re-inits passed. Four natural boots of the scenario on the fixed build did not panic either.

**How it was found.** Not by a test aimed at it: by a routine OTA push whose post-verify read failed, then an SD log that showed two PANIC boots, then the coredump. **Read the boot rows after every push** — `ota_push.py` reported success from the third, healthy boot. **Decoding the dump without IDF's tools:** it is an ELF core; the `ESP_EXTRA_INFO` note gives the crashed TCB and `EXCCAUSE`/`EXCVADDR`, each `CORE` note's registers start 72 bytes in (PC first, `a0..a15` 256 bytes after that), a TCB's name is at offset 52, and the toolchain's `xtensa-esp32s3-elf-addr2line` resolves the rest.

**Two traps met while proving it.** (1) **A task that measures free heap just before it deletes itself counts its own stack**, and FreeRTOS frees a deleted task's memory later, from the idle task — so the first stress runs "leaked" 2–10 KB that were back seconds later. Measure from outside, after a settle. (2) **An unlocked driver delete does not always fault the same way**: with a companion task reading back to back it locked the board up (`INT_WDT`, *no coredump*); with only T5/T17 traffic it panicked with the field signature. A fail-first test must accept either as the crash, and "no coredump" does not mean "no crash" — read the SD log's `esp_reset_reason` row.

**Where it lives.** `drivers/modBus/src/modbus_rtu.cpp` (`modbus_init`, the fail-first switch, the `reinit` counters), `drivers/modBus/src/modbus_rtu.h` (the `@note`), `firmware/src/diag/modbus_bench.cpp` (the stress runner), `bin/at_modbus_reinit.py`, `firmware/src/main.cpp` (init comment), `design/addModbusMutex.md` §6.1, gh#69.

## 2026-09-16 — the M3 teach "worked" from OPEN because T6 finished it; the success signal was real and belonged to another task

**Problem.** Every teach from the GUI ended *teach complete* beside *INVALID — teach still armed*. Two fixes went in (`3006cce`: judge on a fresh read; `a3d2c2a`: a CHECKING state for a supposed persist race) and the second was flashed and run: from CLOSED, CHECKING never resolved and the teach timed out as FAILED. Meanwhile the SD log of the previous teach, from OPEN, said `teach COMMITTED` — so the teach looked sound and the display looked racy.

**Root cause.** The teach drove **one** traverse from a parked end. The sensor captures an end only when its end sensor **makes** after arming (contract §6.2), so the end the leaf started on could never be captured and the device could never commit on the teach's account. The OPEN teach committed because **T6 reopened the window 11 s later** and made the second capture: `11:49:50` closed sensor made → `11:50:01` T6 `MOVING_OPEN` → `11:50:12` open sensor made → `11:50:13` COMMITTED. T17's ordinary poll — the full 15-register map, which includes `30013`/`30014` — supplied the read handshake. From CLOSED, T6 wanted the window closed, was held by dwell, and the 30 s timeout disarmed the device at `12:01:48`, the same second T6's CLOSE began. The operator's diagnosis was the right one: *"direction should be irrelevant during any teach"*.

**Why it survived two fixes.** The success signal was **genuine** — the device did commit — so nothing contradicted "the teach works, the display is wrong". Nobody asked *which stroke* made the second capture. The "persist race" (848 on screen, 850 taught) was the device still holding its old calibration, read before T6 finished the job.

**Fix.** The teach drives **legs** until two end sensors have made — each leg opposite to T2's belief, since T2 ignores a command to go where it thinks M3 already is — and treats **bit 5 clearing** as the commit. No at-an-end precondition; M3 may start anywhere. `bin/at_wp_teach.py` proves it from both ends on the rig. Found while verifying: the sensor keeps an armed teach across a controller restart, and T17's poll would let T6 complete it unwatched — T17 now aborts any teach the commissioning path does not own (param 245 value 4, `orphan_aborts`). And the first cut of THAT logged the operator's own abort as an orphan, because bit 5 lags the disarm — fixed with a 5 s grace, and the harness now fails if the counter moves.

**The generalisable part.**

- **A success signal proves that something succeeded, not that YOUR action did.** Before crediting an outcome to an action, read the timeline between them: if another actor moved the same thing in between, the credit is unproven. Here the tell was a 23 s gap between the teach's own end (`11:49:50`) and the commit (`11:50:13`), with a T6 stroke inside it.
- **When a symptom resists two fixes, stop fixing the symptom and re-read the contract for the mechanism.** Both fixes were built on a race nobody had observed; the contract said in its step table that a capture needs a sensor *make*.
- **A comment asserting what a task does NOT do is a claim to verify.** `emit_events()` said T17 never reads `30013`/`30014`; its every poll does, and that is exactly how T6 completed the teach.
...
**Root cause, two failures stacked — either alone would have been survivable.**

**(1) A hand-counted capacity limit that described one build variant.** `cfg.max_uri_handlers = 36`, with the comment *"34 routes total, 2 spare"*. True of the **release** build. `MODBUS_BENCH` adds four routes, taking the bench build to **38**. `httpd_register_uri_handler` is called in array order, so slots 35 and 36 went to `diag_modbus` and `diag_windowpos` — which is why `/api/diag/windowpos` worked all session and masked the problem — and **37 and 38 failed**, which were *both* commission routes. `/api/diag/commission` 404s.

**(2) The GUI hid the dependent feature instead of greying it.** `commPoll()` did `card.hidden = !(c && c.ok)`, so a non-ok reply removed the whole **Linear control** group — window size, deadzone, calibration, teach. A hidden control answers no question: the operator cannot tell *"this build lacks it"* from *"wrong tab"* from *"it is broken"*, and has nothing to search for or quote. **The only symptom available was a missing thing, which is the hardest kind to report.**

**Why it survived.** The registration failure logged one `ESP_LOGE` per route to the **serial console** — on a unit in a greenhouse — and the very next line was `ESP_LOGI(... "34 routes registered")`, a hardcoded number asserting a count nothing had checked. So the boot log *confirmed* the thing that had just failed.

**Fix.** The route table is declared **before** the httpd config and the limit is derived from it (`ARRAY_LEN(uris) + 2`), so the arithmetic that drifted is gone rather than corrected. Registration failures are counted and summarised loudly (*"N of M ROUTES FAILED TO REGISTER -- those endpoints will 404 and any GUI depending on them stays hidden"*), and the banner prints the real count. The GUI now greys the group out with the reason beside it (`.disabled-block` / `.disabled-why`), which is **promoted to a design rule in CLAUDE.md**.

**The generalisable part.**

- **A conditional compile block that adds routes, tasks, queue entries or table rows must adjust whatever CAPACITY those live in** — and the capacity should be derived from the collection, never hand-counted. A hand-counted constant is a claim about one build variant, and `#ifdef` makes variants.
- **A registration/insert call that can fail needs its failures counted, not just logged per-item.** One line in a scroll on a console nobody reads is not a report.
- **Second instance in two days of "parameterised or conditional in one place, hardcoded in another"** — the other was `build_release.ps1` building `lolin_s3_bench` and copying `.pio/build/lolin_s3/firmware.bin` (entry below). Both reached hardware. When you make something vary, grep for every place that assumed it did not.
...
**Problem.** A 9 h M2k capture of the RS485 bus decoded 1 598 transactions from addr 1 (FG6485A) and addr 44 (S200) and **not one** from addr 40 (the wire encoder) — while the controller was transacting with addr 40 thousands of times an hour. Two successive explanations were asserted and both were wrong: first *"the probe is not electrically on that traffic"* (a rig-topology claim), then *"the 200 ms capture window truncates the 35-byte reply"*.

**Root cause.** Neither. The **software UART decoder in the capture script** free-runs: it locks on a falling edge, samples 8 bits, emits a byte, jumps a fixed 10 bit-times, and scans for the next edge. It never validates the stop bit, so one bad lock corrupts every byte after it for the rest of the capture.

The encoder replies **~5.2 ms** after the master releases the line (measured, n=26, mean 5 234 µs) against **22 ms** for the FG6485A and **73 ms** for the S200. In that short turnaround the RS485 line is still settling as the driver hands over, and the glitch decodes as a spurious byte — sometimes `ff`. Locking onto it shifts the real reply by one bit: `0x28` comes out as `0x50` (`0x28 << 1`, the start bit shifted into the LSB). In some captures the phantom byte even **bridged the request/reply gap**, so `split_frames()` merged them into one frame and the CRC could never pass.

Requests to addr 40 decoded perfectly throughout (`28040000000fb7f7`, CRC ok) — the master drives the line cleanly. **It was never "addr 40 cannot be decoded"; it was "that device's REPLY cannot be decoded".** The decoder does not know about addresses at all.

**Fix.** Partial. Validating the stop bit (reject the byte and rescan if it is low) corrects the bit alignment — `28 04` now decodes where `50 04` and `30 04` did. CRC still fails and frame lengths are still wrong, so the encoder's latency remains unmeasured. Finishing it needs the analog waveform across the turnaround inspected, not more iteration on decoded bytes.

**The part worth remembering.** `GET /api/diag/windowpos` reported **`crc 0`, `framing 0`** on 5 400+ reads of addr 40 the entire time. The bus was clean, the firmware's own decoder was correct, and every minute of this was an instrument problem. A real RS485 receiver with proper biasing tolerates a turnaround that a fixed-threshold single-ended probe cannot.

**Where it lives.** `m2k_soak.py` in the session scratchpad — not repo code. `drivers/modBus/src/modbus_rtu.cpp` needed no change and got none.

**Constraint for any future bus measurement on this rig:** a single-ended threshold probe cannot decode a reply that arrives within a few ms of the master releasing the line. Measuring the encoder needs differential probing across A/B with bias, or a decoder that re-locks per byte. The two emulated slaves are easy targets precisely because they are slow.
